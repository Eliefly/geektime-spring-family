...
import geektime.spring.data.mybatis.mapper.manual.CoffeeDao;
...
//        generateArtifacts();
    ...
    private void playWithArtifacts() {
        ...
        Coffee latte = new Coffee()
                ...
                .withCreateTime(new Date())
                .withUpdateTime(new Date());
        coffeeDao.insert(latte);

        Coffee s = coffeeDao.selectByPrimaryKey(1L);
        log.info("Coffee {}", s);

        CoffeeExample example = new CoffeeExample();
        example.createCriteria().andNameEqualTo("latte");
        List<Coffee> list = coffeeDao.selectByExample(example);
        list.forEach(e -> log.info("selectByExample: {}", e));

        Coffee coffee = coffeeDao.findByName("espresso");
        log.info("coffee find by name {}", coffee);
